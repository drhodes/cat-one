...
- **Example**: `mcp_libspec_search(query="user authentication")`
...
- **Example**: `mcp_libspec_peek(file_path="auth.py", line=42, character=10)`

### 🏗️ mcp_libspec_symbols
...
- **Example**: `mcp_libspec_symbols(file_path="models.py")`
...
- **Example**: `mcp_libspec_usage(file_path="utils.py", line=12, character=5)`
...
7. **Automated VCS Linking (Do NOT manually link)**: Git commit hooks automatically manage VCS linking (`libspec link` / `mcp_libspec_link_snapshot`) under the hood during the commit process. Because these hooks handle linking automatically, you (the coding agent) **must NOT manually call `libspec link` or invoke the `mcp_libspec_link_snapshot` tool**. Running the `libspec link` command manually is not usually done; rather, it should be avoided unless there are extenuating repairs needed, because executing it manually breaks the libspec developer workflow (e.g., by bypassing git-hook boundaries and leaving the workspace in a dirty state).
